...
import { protobufHandler } from './protobuf';

// 云湖API基础URL
const BASE_URL = 'https://chat-go.jwzhd.com';

// 初始化ProtoBuf
protobufHandler.initialize().catch(console.error);
...
const protobufRequest = async (url: string, messageType: string, data?: any, responseType?: string) => {
  try {
    ...
    if (data) {
      body = protobufHandler.serialize(messageType, data);
    }
    ...
    if (responseType) {
      return await protobufHandler.parseResponse(responseType, response);
    } else {
      // 如果没有指定响应类型，尝试解析为通用状态响应
...
export const userAPI = {
  ...
  getUserInfo: async () => {
    try {
      const response = await protobufRequest('/v1/user/info', '', undefined, 'UserInfo');
      return response;
    } catch (error) {
      throw error;
    }
...
export const conversationAPI = {
  ...
  getConversationList: async () => {
    try {
      const response = await protobufRequest('/v1/conversation/list', '', undefined, 'ConversationList');
      return response;
    } catch (error) {
      throw error;
    }
